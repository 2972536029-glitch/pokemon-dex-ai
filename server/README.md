...
> 配套 `../src/chat/`(前端对话面板)与 `./src/`(Node 服务端)。
> 代码注释是行级说明,本文讲设计层的 why。产品演进方向见 `./docs/ROADMAP.md`。

## 一句话定位

在宝可梦图鉴上加的 AI 问答助手:用户用自然语言提问,模型通过 **Function Calling** 实时调
PokeAPI 查数据后回答,回答流式输出,并有一层**幻觉校验**对数值与属性克制断言做机械核对。
...
API key 只在服务端。前端只渲染 SSE 事件流,不参与工具编排。
...
- **错误是数据不是异常**:404 回给模型 `{error, hint}`,模型可以自己重试
  (比如把中文名换掉),循环不至于崩。
...
三道闸:
1. **MAX_ROUNDS = 6**:模型反复要工具的"失控螺旋"被硬切断;
2. **AbortSignal 全链路**:用户关面板/断网,`res` 的 close 触发 abort,
...
2. 最终回答按句切分,两类断言会被核对:
   - **数值断言**:句子提到某只宝可梦 + 属性关键词(HP/攻击/体重…)+ 数字,
     拿数字对事实表(统计句继承上一句的主语);
   - **类型断言**:"A 克 B" / "B 怕 A" / "B 的弱点是 A" 两个方向,对照已查询
     的 `damage_relations`;
...
边界要诚实:校验器只管最容易被编、也最容易机械验证的那部分;更开放的语义断言
是 v2 方向(见 ROADMAP)。
...
- PokeAPI 超时/5xx → 重试一次后 `upstream_unavailable`,模型如实转告;
...
SDK 会把 tool_calls 分片拼装、SSE 解析、循环控制藏起来。反过来也成立:
产品化、多模型路由、复杂 RAG 链时,自研维护成本不划算——取舍是场景定的。
